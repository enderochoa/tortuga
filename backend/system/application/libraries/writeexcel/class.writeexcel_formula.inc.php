...
function _match($token)
    ...
    switch($token)
 {
        ...
        default:
            ...
            if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/',$token) and
               (preg_match("/[0-9]/",$this->_lookahead)==0) and 
                   ...
                   ($this->_lookahead != ':') and ($this->_lookahead != '.'))
 {
            ...
            elseif (preg_match("/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+:(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/",$token) and 
                   (preg_match("/[0-9]/",$this->_lookahead)==0))
 {
                return $token;
            }
            // if it's a range (A1..A2)
            ...
            elseif (preg_match("/^\"[^\"]{0,255}\"$/",$token))
 {
            ...
            elseif (preg_match("/^[A-Za-z0-9\xc0-\xdc\.]+$/",$token)>0 and ($this->_lookahead == "(")) {
                return $token;
            }
            return '';
    }
...
function _expression()
 {
    ...
    if (preg_match("/^\"[^\"]{0,255}\"$/", $this->_current_token)>0)
 {
...
function _fact()
 ...
 elseif (preg_match("/^[A-Za-z0-9\xc0-\xdc\.]+$/",$this->_current_token)>0)
 {
    // if it's a function call
        $result = $this->_func();
        return $result;
...
function _func()
 {
    ...
    while ($this->_current_token != ')'){
      if ($num_args > 0){
        ...
        trigger_error("Sintactic error: coma expected in "."function $function, {$num_args}� arg", E_USER_ERROR);
      }
