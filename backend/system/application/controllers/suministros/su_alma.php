...
//almacenes
class su_alma extends Controller {

	var $url  ='suministros/su_alma';
	var $titp ='Receptores';
	var $tits ='Receptor';
	...
	function index(){
		$this->db->query("INSERT IGNORE INTO su_alma (`codigo`,`descrip`) VALUES ('0000','Sin Asignar')");
	...
	function filteredgrid(){
		...
		$filter = new DataFilter("Filtro de ".$this->titp,"su_alma");
		...
		$grid->column_orderby("U. Ejecutora"        ,"uejecuta" ,"uejecuta"  );
	...
	function dataedit(){
		...
		$edit = new DataEdit($this->tits, "su_alma");
		...
		$edit->gralma = new dropdownField("Grupo de Receptor", "gralma");
		$edit->gralma->options("SELECT codigo, descrip FROM su_gralma ORDER BY descrip");
		$edit->gralma->style="width:250px";
	...
	function _post_insert($do){
		...
		logusu('su_alma',$this->tits." $codigo CREADO");
	}
	function _post_update($do){
		$codigo=$do->get('codigo');
		logusu('su_alma',$this->tits." $codigo  MODIFICADO");
	}
	function _post_delete($do){
		$codigo=$do->get('codigo');
		logusu('su_alma',$this->tits." $codigo  ELIMINADO ");
	}
