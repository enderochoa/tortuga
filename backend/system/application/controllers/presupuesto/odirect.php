...
class odirect extends Common {
	...
	function filteredgrid(){
		...
		$filter->status->option("B" ,"Pendiente"    );
		$filter->status->option("BY","Reverso"      );
		$filter->status->option("B2","Causado"    );
		$filter->status->option("B1","Por Causar");
		$filter->status->option("B3","Pagado"       );
		$filter->status->option("BA","Anulado"      );
		...
		function sta($status){
			switch($status){
				case "B1":return "Por Causar";break;
				case "B2":return "Causado";break;
				case "B3":return "Pagado";break;
				case "B":return "Pendiente";break;
				case "BY":return "Reverso";break;
				case "BA":return "Anulado";break;
			}
		}
...
//		echo $grid->db->last_query();
	...
	function dataedit($back='',$status='',$numero=''){
		...
		$do->pointer('nomina' ,'nomina.nomi = odirect.nomina','nomina.denomi as denomin','LEFT');
		...
		$edit->nomina->size     = 15;
		$edit->nomina->append($bNOMINA);

		$edit->denomin = new inputField("", 'denomin');
		$edit->denomin->size     = 60;
		$edit->denomin->readonly = true;
		$edit->denomin->pointer  = true;
		$edit->denomin->in       = "nomina";
		...
		$edit->status->option("","");
		$edit->status->option("B2","Actualizado");
		$edit->status->option("B1","Sin Actualizar");
		$edit->status->option("B3","Pagado");
	...
	function _valida($do){
		...
		if(!empty($cod_prov)){
				//$do->set('reten'     ,    $treten     );
				$reteiva=round((($giva+$riva+$aiva)*$reteiva_prov)/100,2);
		}else{
			$reteiva=0;
		}

		$total2=$giva+$riva+$aiva+$subtotal;
		$total =$total2-$reteiva-$reten-$retenomina-$otrasrete;

		$impm=$impt=0;
		if($do->get('simptimbre')=='S')
			$total       -=$impt=($subtotal * $this->datasis->traevalor('IMPTIMBRE')/100);
		...
		if( ($reteiva+$reten >0) AND $multiple=='N'){
	...
	function _post_insert($do){
		$numero = $do->get('numero');
		logusu('odirect',"Creo Orden de Pago Contrato Nro $numero");
		//redirect($this->url."actualizar/$numero");
	}
	function _post_update($do){
		$numero = $do->get('numero');
		logusu('odirect'," Modifico Orden de Pago Contrato Nro $numero");
		//redirect($this->url."actualizar/$numero");
	}
	function _post_delete($do){
		$numero = $do->get('numero');
		logusu('odirect'," Elimino Orden de Pago Contrato Nro $numero");
	}
	...
	function instalar(){
			...
			$this->db->simple_query("ALTER TABLE `itodirect` CHANGE COLUMN `partida` `partida` VARCHAR(25) NULL DEFAULT NULL COMMENT 'Partida Presupuestaria'  ");
	    $this->db->simple_query("ALTER TABLE `odirect`  ADD COLUMN `simptimbre` CHAR(1) NULL DEFAULT 'N' AFTER `montocontrato`");
	}
}
?>
