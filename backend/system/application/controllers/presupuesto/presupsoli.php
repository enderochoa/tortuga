<?php
class presupsoli extends Controller {
	var $formatopres;
	var $flongpres;
	var $formatoadm;
	var $flongadm;
	...
	function index() {
		$this->datasis->modulo_id(62,1);
		redirect("presupuesto/presupsoli/sel");
	}
	...
	function asignar($tipo,$codigoadm,$codigopres=null)
	{
		...
		function asigna($valor,$value=null){//$tipo,$codigoadm,$codigopres,
		...
		$tabla=form_open("presupuesto/presupsoli/carga/");
		...
		$grid = new DataGrid("Estimaci&oacute;n del Presupuesto");
	...
	function carga(){
		...
		$data['content'] = 'Guardado'.anchor('presupuesto/presupsoli','regresar');
		$data['title']   = "Reporte";
		$data["head"]    = script("jquery.js").script("plugins/jquery.numeric.pack.js").$this->rapyd->get_head();
		$this->load->view('view_ventanas', $data);
