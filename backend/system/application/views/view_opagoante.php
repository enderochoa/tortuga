...
if($form->_status!='show'){
	...
	$idt  =$this->datasis->dameval("SELECT id FROM modbus WHERE idm='v_presaldoante' AND uri='$uri'");
...
<style >
	...
	$(document).ready(function() {
		...
		$("#mivag").focusout(function(){
			cal_mivag();
		});
		
		$("#mivar").focusout(function(){
			cal_mivar();
		});
		
		$("#mivaa").focusout(function(){
			cal_mivaa();
		});
		
		$("#imptimbre").focusout(function(){
			cal_total();
		});
		/*
	...
	function cal_timporte(){
		...
		$("#mivaa").val(Math.round(aiva*100)/100);
		$("#mivar").val(Math.round(riva*100)/100);
		$("#mivag").val(Math.round(giva*100)/100);
	...
	function cal_total(){
		...
		riva=parseFloat($("#ivar").val());
		amortiza=parseFloat($("#amortiza").val());
		...
		reteiva=Math.round((((giva+riva+aiva)*reteiva_prov)/100)*100)/100;
		tot=mivag+mivar+mivaa+giva+riva+aiva;
		
		reten=$("#reten").val();
		
		to=tot-reteiva-reten-retenomina-imptimbre-otrasrete-amortiza;
			...
			<table width="100%"  style="margin:0;width:100%;">
     		...
     		<tr> 
			  ...
			  <tr>
			    ...
			    <td class="littletablerowth"><?=$form->amortiza->label ?>&nbsp;</td>
			    <td class="littletablerow"  ><?=$form->amortiza->output?>&nbsp;</td>
			  </tr>
			</table><br />
			<table width='100%'>
				<tr>
				...
				<tr>
					...
					<td class="littletablerow" align='right' colspan="4"><?=$form->mivag->label.$form->mivag->output    ?>              </td>
				...
				<tr>
					...
					<td class="littletablerow" align='right' colspan="4"><?=$form->mivar->label.$form->mivar->output  ?></td>
				...
				<tr>
					...
					<td class="littletablerow" align='right' colspan="4"><?=$form->mivaa->label.$form->mivaa->output ?></td>
				...
				<tr>
					...
					<td class="littletablerow" align='right' colspan="4"><?=$form->mexento->label.$form->mexento->output ?></td>
