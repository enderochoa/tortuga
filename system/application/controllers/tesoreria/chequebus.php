<?php
class chequebus extends Controller {
	...
	function index(){
		...
		$filter = new DataForm('tesoreria/chequebus/index/process');
		...
		$tablas='';
		 if ($filter->on_success()){
			...
			$grid2->db->select(array("a.observacaj","numero","proveed","b.nombre","rif","fecha","total","a.reteiva","reten","imptimbre","total2","a.observa","status","
			...
			$grid2->column('Observaci&oacute;n','<opobservac><#numero#>|<#observacaj#></opobservac>',"align='left' " );
			$grid2->build();
			}
			...
			$grid3->db->where('a.cod_prov',$cod_proveed);
			...
			$grid3->column('Cheque'       ,'cheque');
			$grid3->column('Monto'        ,'<nformat><#total#></nformat>',"align='right'");
			$grid3->column('Observasion'  ,'observa'                     ,"align='left' NOWRAP");
			$grid3->build();*/
		...
		$data['title']   = 'Consulta de pagos en caja';
		$data['head']    = $this->rapyd->get_head();
		$this->load->view('view_ventanas', $data);
	}
	
	function observacaj(){
			$observacaj =$this->input->post("observacaj");
			$id         =$this->input->post("id");
			$observacaje=$this->db->escape($observacaj);
			$ide        =$this->db->escape($id        );
			
			echo "UPDATE mbanc SET observacaj=$observacaje WHERE id=$ide";
			$this->db->query("UPDATE mbanc SET observacaj=$observacaje WHERE id=$ide");
	}
	
	function opobservacaj(){
			$observacaj =$this->input->post("observacaj");
			$id         =$this->input->post("id");
			$observacaje=$this->db->escape($observacaj);
			$ide        =$this->db->escape($id        );
			
			$this->db->query("UPDATE odirect SET observacaj=$observacaje WHERE numero=$ide");
	}
