<?php
class Forma1 extends Controller{
	 ...
	 *
	 * */
	function agrega_col($camp,$name,$pos,$tam=0,$alinea="",$tfuen,$estilo="",$fillColor=0,$tdato='',$reseth=true,$stretch = 0,$autopadding = true,$valign = 'T',$border='T',$height=4,$flineal=array('depen_campo'=>false,'depen_valor'=>null,'style'=>'B')){

		$this->columnas[]=array('campo'=>$camp,'titulo'=>$name,'pos'=>$pos,
								"ancho"=>$tam,'alinea'=>$alinea,'tfuente'=>$tfuen,
								'estilo'=>$estilo,'borde'=>$border,'relleno'=>$fillColor,'tdato'=>$tdato,
								'reseth'=>$reseth,'stretch'=>$stretch,
								'autopadding'=>$autopadding,'valign'=>$valign,'height'=>$height,'flineal'=>$flineal);
	}
	/*
	 * Funcion Para mostrar una Celda con formato de titulo
	 */
	function titulo($titulo,$pdf,$objt,$fondo="",$tamLetra,$aling="",$actfondo,$border='1'){
		$objt->SetFont($this->fuenteTabla, 'B', $tamLetra);
		if($fondo=="") $fondo=5;
		$objt->setFillColor(0,0,0,$fondo);

		$objt->Cell(0, 4, $titulo, $border, 0, $aling, $actfondo);
	 ...
	 */
	function set_totalizar($camp,$pos,$tam=0,$alinea="",$borde=0,$fillColor=0,$tfuen,$estilo="",$valor=0,$fijo=false){

		$this->totalizar[]=array('campo'=>$camp,'pos'=>$pos,
								"ancho"=>$tam,'alinea'=>$alinea,'tfuente'=>$tfuen,
								'estilo'=>$estilo,'borde'=>$borde,'relleno'=>$fillColor,
								'valor'=>$valor,'fijo'=>$fijo);
	}
	...
	function set_acumulador($camp,$valor=0){

		$this->totalacu[]=array('campo'=>$camp,'valor'=>$valor);
...
//$objt->StartTransform();
//$objt->Rotate(90);
			$objt->MultiCell($colum['ancho'], $colum['height'], $colum['titulo'], $colum['borde'], 'C',1,0, $x = '', $y = '', $reseth = true, $stretch = 0,$ishtml = true,$autopadding = true,$maxh = 0);
...
//$objt->StopTransform();
		...
		foreach ($this->tabla AS $items){
			...
			if($objt->getY()+$mayor+$espCuerpo >= $this->limite){
				...
				foreach($objetos as $objT){
					...
					foreach ($this->columnas as $colum){
						...
						$$objT->MultiCell($colum['ancho'], $colum['height'], $colum['titulo'], 1, 'C',1,0, $x = '', $y = '', $reseth = true, $stretch = 0,$ishtml = true,$autopadding = true,$maxh = 0);
					}
			...
			foreach($objetos as $objT){
				...
				foreach ($this->columnas as $colum){
					...
					//echo $elemento."<br>";
					if($elemento===true)$elemento=$valorCampo[$ncamp];
					//echo $elemento."<br>=><br>";
					if($colum['tdato']!=""){
						if($colum['tdato']==1)$valor=(is_numeric($elemento) || strlen($elemento)==0 ?nformat($elemento):$elemento);
						if($colum['tdato']==2)$valor=dbdate_to_human($elemento);
						if($colum['tdato']==3)$valor=($elemento==0?'':nformat($elemento));
						if($colum['tdato']==4)$valor=($elemento==''?'':nformat($elemento));
					}else{
						$valor=$elemento;
					}
					
					if($flineal['depen_campo']){
						if($items[$flineal['depen_campo']]==$flineal['depen_valor']){
							$est=$flineal['style'];
						}
					}
	...
	function dibuja_total($pdfa,$bandTexto){
		...
		$pdfa->setX(100);
		if(isset($this->TotalizarPP['ingreso']))
$pdfa->cell(40,4,nformat($this->TotalizarPP['ingreso']),0,0);
	}
	...
	function dibuja_totalizar($pdfa){
		...
		foreach($this->totalizar as $total){
			...
			if(is_numeric($total['fijo']))
			$monto=nformat($total['fijo']);
			else
			$monto=nformat($total['valor']);
	 ...
	 */
	function dibujaArreglo($pdf,$objt,$camb=0,$fin=0,$arreglo=null){
		if(!$arreglo)
		...
		foreach($arreglo as $lineas){
			foreach($objetos as $objT){
				...
				foreach($lineas as $c){
					...
					$autopadding=$c[2][7];
					if(isset($c[2][8][1]))
					$$objT->SetFillColor($c[2][8][0],$c[2][8][1],$c[2][8][2]);
					$$objT->setFont($this->fuenteTabla,$est , $tamLetra);
