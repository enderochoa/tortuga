...
define ("K_PATH_MAIN", "/srv/www/htdocs/ci4/");
...
if(!class_exists('TCPDF')) {
	...
	*/
	class Tcpdf {
		...
		var $page;   			// @var current page number
		var $n; 			// @var current object number
		var $offsets; 			// @var array of object offsets
		var $buffer; 			// @var buffer holding in-memory PDF
		var $pages; 			// @var array containing pages
		var $state; 			// @var current document state
		var $compress; 			// @var compression flag
		var $DefOrientation; 		// @var default orientation
		var $CurOrientation; 		//  @var current orientation
		var $OrientationChanges;  	// @var array indicating orientation changes
		var $k; 			// @var scale factor (number of points in user unit)
		var $fwPt; 			// @var width of page format in points

		/**
		* 
		* @access protected
		*/
		var $fhPt; // @var height of page format in points
		...
		*/
		...
		var $AutoPageBreak; // @var automatic page breaking
		...
		*/
		 ...
		 */
		function Tcpdf($orientation='P', $unit='mm', $format='LETTER', $unicode=true, $encoding="UTF-8") {
		...
		function SetFont( $family, $style='', $size=0) {
			...
			if(!isset($this->fonts[$fontkey])) {
				...
				if(isset($this->CoreFonts[$fontkey])) {
					...
					$this->Error('1 Undefined font: family ('.$family.') Style ('.$style.') path '.K_PATH_MAIN);
				}
			}
			//Select it
      ...
      echo "</br>a".$this->y;
		...
		*/
		function _puttruetypeunicode($font) {
			// Type0 Font
			// A composite fontâa font composed of other fonts, organized hierarchically
			$this->_newobj();
			$this->_out('<</Type /Font');
			...
			// Font descriptor
			// A font descriptor describing the CIDFontâs default metrics other than its glyph widths
			$this->_newobj();
