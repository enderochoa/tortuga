...
$campos=$form->template_details('r_cxcit');
	
$scampos  ='<tr id="tr_r_cxcit_<#i#>">';
...
<script language="javascript" type="text/javascript">
	reciboit_cont=<?=$form->max_rel_count['r_cxcit'] ?>;
	rifci_g=null;
	...
	$(document).ready(function(){
		...
		$( "#b_cargadeudainmueble" ).button().click(function( event ) {
			...
			$.ajax({
				url: "<?=site_url('recaudacion/r_cxc/inmueble_cant')?>",
			  type: 'POST',
			  async: false,
			  data: { 
		...
		$( "#b_cargadeudavehiculo" ).button().click(function( event ) {
			...
			$.ajax({
				url: "<?=site_url('recaudacion/r_cxc/vehiculo_cant')?>",
			  type: 'POST',
			  async: false,
			  data: {
				  id_contribu:id_contribuv 
		...
		$.post("<?=site_url('recaudacion/r_cxc/dameconc')?>",{ partida:"" },function(data){
	...
	function autoconcdenomi(){
		$("input[name^='denomi_']").focus(function(){
				...
				select: function( event, ui ) {
					...
					if(pos>0){
						...
						if(ui.item.requiere=='INMUEBLE'){
							...
							if(id_contribuv>0){
								$.ajax({url: "<?=site_url('recaudacion/r_cxc/inmueble_cant')?>", type: 'POST', async: false,
								data: {
									id_contribu:id_contribuv
									}
								}).done(function(data){
								...
								if(cantidad==1){
									$.ajax({url: "<?=site_url('recaudacion/r_cxc/inmueble_get') ?>", type: 'POST', async: false,
										data:{
											id_contribu:id_contribuv
										}
									}).done(function(data){
							...
							if(ui.item.requiere=='VEHICULO'){
								...
								if(id_contribuv>0){
									$.ajax({url: "<?=site_url('recaudacion/r_cxc/vehiculo_cant')?>", type: 'POST', async: false,
									data: {
										id_contribu:id_contribuv
										}
									}).done(function(data){
									...
									if(cantidad==1){
										$.ajax({url: "<?=site_url('recaudacion/r_cxc/vehiculo_get') ?>", type: 'POST', async: false,
											data:{
												id_contribu:id_contribuv
											}
										}).done(function(data){
			...
			.data( "autocomplete" )._renderItem = function( ul, item ) {
				return $( "<li></li>" )
				.data( "item.autocomplete", item )
				.append( "<a>" + item.id+'.'+item.ano + " - " +item.frecuenciatexto+" "+item.freval+" "+ item.denomi+ "</a>" )
				.appendTo( ul );
			};
		});
	}
  
	function post_conc(i,tm){
	...
	function add_r_cxcit(){
		...
		post_conc(reciboit_cont,1);
		reciboit_cont=reciboit_cont+1;
		cal_total();
		autoconcdenomi();
	}
	
	function del_r_cxcit(id){
		
		id = id.toString();
		$('#tr_r_cxcit_'+id).remove();
		cal_total();
	}
	...
	<tr>
		<td>
			...
			<table   border="0" cellpadding="0" cellspacing="0" class="table_detalle">
				...
				<?php 
				for($i=0;$i<$form->max_rel_count['r_cxcit'];$i++) {
					$obj0="itid_concit_$i";
					$obj1="itdenomi_$i";
					$obj3="itano_$i";
					$obj4="itobserva_$i";
				...
				?>
				 <tr id='tr_r_cxcit_<?=$i ?>'>
					<td class="littletablerow"              ><?=$form->$obj11->output.$form->$obj9->output.$form->$obj17->output.$form->$obj0->output.$form->$obj1->output.$form->$obj14->output ?></td>
					...
					<td class="littletablerow"              ><div id="temporal_<?=$i ?>" ></div><?=$form->$obj4->output ?></td>
					<td class="littletablerow" align='right'><?=$form->$obj15->output ?></td>
					<td class="littletablerow" align='right'><?=$form->$obj5->output ?></td>
					<?php if($form->_status!='show') {?>
					<td class="littletablerow" align="center"><a href=# onclick='del_r_cxcit(<?=$i ?>);return false;'><?=image('delete.jpg','#',array("border"=>0))?></a><span class="papelerait">_._<?=$i?>_._</span></td>
					<?php } ?>
