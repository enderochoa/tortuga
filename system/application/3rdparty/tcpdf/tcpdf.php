	...
	class TCPDF {
		...
		public function Footer() {
			...
			if (empty($this->pagegroups)) {
				$pagenumtxt = ($this->textpie[3]===false?'':$this->textpie[3].' '.$this->getAliasNumPage().' / '.$this->getAliasNbPages());
			} else {
				$pagenumtxt = ($this->textpie[3]===false?'':$this->textpie[3].' '.$this->getPageNumGroupAlias().' / '.$this->getPageGroupAlias());
			}
			$this->SetY($cur_y);
			//Print page number
			if ($this->getRTL()) {
				...
				$this->SetX($ormargins['left']);
				$final=$this->pagedim[$this->page]['wk'];$final=$final-15;
				$this->Line(15,$this->getY(),$final,$this->getY());
				$a=($final-15)/3;
				$this->Cell($a, 0, $this->textpie[1], 0, 0, 'L');
				$this->Cell($a, 0, $this->textpie[2], 0, 0, 'C');
				$this->Cell($a, 0, $pagenumtxt, 0, 1, 'R');
				$this->Cell(0, 0, $this->textpie[4], 0, 0, 'C');
			}
		}
		...
		protected function _datestring() {
			...
			$current_time = date_default_timezone_set('America/Caracas');
